...
const Home = () => {
  ...
  return (
    <main className="text-white w-full h-[100vh] overflow-hidden grid grid-cols-1 md:grid-cols-12 bg-gray-900">
      ...
      <aside className={`col-span-12 md:col-span-4 bg-gray-800 border-r border-gray-700 h-full overflow-hidden ${chatId !== "null" ? 'hidden md:block' : 'block'}`}>
        <Sidebar />
      </aside>

      {/* Chat Area: Hidden on mobile if no chat selected */}
      <section className={`col-span-12 md:col-span-8 bg-gray-900 h-full overflow-hidden ${chatId === "null" ? 'hidden md:block' : 'block'}`}>
        <Chat />
      </section>
